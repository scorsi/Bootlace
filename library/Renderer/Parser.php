...
namespace Bootlace\Renderer;

use Bootlace\Renderer\Plugin\PluginContainer;
use Src\TemplateEngine\SyntaxeErrorException;
...
class Parser
{
    ...
    )
    {
        ...
        if (flock($fp, LOCK_EX)) {
            ...
            $parsedCode = "<?php if(!class_exists('Bootlace\\Renderer\\Renderer')){exit;}?>" . $parsedCode;
    ...
    protected function compileTemplate($code, $isString, $templateBasedir, $templateDirectory, $templateFilepath)
    {
        ...
        if ($isString) {
            if ($openIf > 0) {
                ...
                $e = new SyntaxeErrorException("Error! You need to close an {if} tag in the string, loaded by {$caller['file']} at line {$caller['line']}");
            ...
            if ($loopLevel > 0) {
                ...
                $trace = debug_backtrace();
                $caller = array_shift($trace);
                $e = new SyntaxeErrorException("Error! You need to close the {loop} tag in the string, loaded by {$caller['file']} at line {$caller['line']}");
                throw $e->templateFile($templateFilepath);
            }
        } else {
            if ($openIf > 0) {
                $e = new SyntaxeErrorException("Error! You need to close an {if} tag in $templateFilepath template");
                throw $e->templateFile($templateFilepath);
            }

            if ($loopLevel > 0) {
                $e = new SyntaxeErrorException("Error! You need to close the {loop} tag in $templateFilepath template");
                throw $e->templateFile($templateFilepath);
            }
        }
    ...
    protected function blackList($html)
    {
        ...
        if (preg_match($this->config['black_list_preg'], $html, $match)) {
            ...
            $e = new SyntaxeErrorException('Syntax ' . $match[0] . ' not allowed in template: ' . $this->templateInfo['template_filepath'] . ' at line ' . $line);
